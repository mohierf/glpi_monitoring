...
class PluginMonitoringDisplayview_rule extends CommonDBTM
{
    ...
    {
        return _n('Monitoring view rule', 'Monitoring view rules', $nb);
    }
     ...
     * @return bool
     */
    static function cronReplayallviewrules($task)
    {
        // todo: really necessary?
//        ini_set("max_execution_time", "0");

        $pmDisplayview_rule = new self();
        $a_rules = $pmDisplayview_rule->find();
        $task->log("Rules count: " . count($a_rules));
        foreach ($a_rules as $data) {
            $task->log("- running: " . print_r($data, true));
            $pmDisplayview_rule->getFromDB($data['id']);
            $pmDisplayview_rule->getItemsDynamically($pmDisplayview_rule);
            $task->log("- ok");
        }

        return max(1, count($a_rules));
    }
    ...
    function ShowRulesTabs($displayviews_id, $tabnum)
    { // Verified
        global $CFG_GLPI;
    ...
    function showReplayRulesForm($displayviews_id, $options = [])
    {
